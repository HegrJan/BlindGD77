...
4.3. When the SK1+hash combination is invoked to associate the last few seconds of DMR audio with a contact, we now suppress continuous DMR save while this dialog is invoked so that the audio buffer is not overwritten, giving the user time to either listen to and save the contact, or edit it and save it. I do not automatically switch to audio edit mode as it would be confusing for the user to have to press green twice, even if the audio is already correct. As soon as the user leaves this screen, normal DMR autosave is restored. Note there is always an advantage to editing the audio immediatley rather than after it has already been saved because the immediate audio buffer is always larger and captures more audio. Once the voice tag has been saved, recalling it later for editing will only allow you to edit what was saved, which will generally be shorter than what was originally captured.
